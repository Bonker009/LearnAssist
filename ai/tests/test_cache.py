...
async def test_key_ignores_resource_order(monkeypatch):
    monkeypatch.delenv("REDIS_URL", raising=False)
    a, b = uuid4(), uuid4()
    assert await cache._answer_key([a, b], "q") == await cache._answer_key([b, a], "q")


async def test_key_differs_when_a_resource_is_added(monkeypatch):
    """Attaching another file can change the answer, so it must not reuse the old one."""
    monkeypatch.delenv("REDIS_URL", raising=False)
    a = uuid4()
    assert await cache._answer_key([a], "q") != await cache._answer_key([a, uuid4()], "q")
